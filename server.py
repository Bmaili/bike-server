...
class Server(object):
    ...
    def request_login(self, client_sock, addr):
        ...
        if (request_data[0] == USER_REQUEST_LOGIN or request_data[0] == BIKE_REQUEST_LOGIN) \
            ...
            login_ID = request_data[1]
            userorBIKE_password = request_data[2]

            # 读取数据库中的信息
            db_conn = DB()
            results = db_conn.select_db(login_ID)

            # 判断接入的是车辆还是用户
            RESULT_LOGIN = USER_RESULT_LOGIN if 'u' in login_ID else BIKE_REQUEST_LOGIN

            if results:

                USERorBIKE_ID = login_ID
                # 查看这个用户是否已经在线
